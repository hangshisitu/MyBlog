...
class Field(object):
    ...
    def __str__(self):
        s = ['<%s:%s,%s,default(%s),' % (self.__class__.name,self.name,self.dll,self._default)]
...
class VersionField(Field):
    ...
    def __int__(self,name=None):
        super(VersionField,self).__init__(name=name,default=0,dll='bigint')
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k,v in attrs.iteritems():
            logging.info('Field %s' % k)
            if isinstance(v,Field):
                if not v.name:
                    v.name = k
                logging.info('Found mapping: %s => %s' % (k,v.name))
        ...
        logging.info('sql:%s' % attrs['__sql__'])
